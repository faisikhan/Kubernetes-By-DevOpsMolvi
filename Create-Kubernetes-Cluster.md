...
> **Note:** The following commands use the official `pkgs.k8s.io` repository. The legacy `apt.kubernetes.io` repository was deprecated in September 2023 and should no longer be used.

12. `curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.35/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg`
13. `echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.35/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list`
14. `sudo apt update`
15. `sudo apt install -y kubelet=1.35.5-1.1 kubeadm=1.35.5-1.1 kubectl=1.35.5-1.1`
...
19. ```
    ...
    ```
20. `sysctl --system`
21. `kubeadm init --pod-network-cidr=10.244.0.0/16` ==> Run this command only on Master node.

> **Tip:** If `kubeadm init` fails, you can reset and try again with:
> `sudo kubeadm reset -f && sudo rm -rf $HOME/.kube`
...
> **Note:** The join token expires after **24 hours**. If it has expired, regenerate it on the master node with:
